...
export class HomeComponent implements OnInit {
  ...
  seekerFilterMode: 'all' | 'saved' = 'all';
 ...
 // Structured categories matching your HTML template
  ...
  // Application Modal & Custom Cover Letter State
  ...
  ngOnInit() {
    if (isPlatformBrowser(this.platformId)) {
      ...
      this.seekerName = localStorage.getItem('seekerName') || '';
      this.loadSavedJobs();
  ...
  loadSavedJobs() {
    ...
    if (this.isSeekerLoggedIn && email !== 'guest') {
      this.http.get<any>(`http://localhost:3000/api/seeker/saved-jobs/${encodeURIComponent(email)}`).subscribe({
        next: (res) => {
          if (res && res.success && Array.isArray(res.savedJobIds)) {
  ...
  toggleSaveJob(job: any) {
    ...
    const email = localStorage.getItem('seekerEmail') || 'guest';
    const targetId = String(job.id || job._id || '');
    const isCurrentlySaved = this.savedJobIds.some((id) => String(id) === targetId);

    if (isCurrentlySaved) {
      this.savedJobIds = this.savedJobIds.filter((id) => String(id) !== targetId);
    } else {
      this.savedJobIds.push(job.id || job._id);
    }
    ...
    if (this.seekerFilterMode === 'saved') {
      this.applyFilters();
    }
    this.cdr.detectChanges();
    ...
    if (this.isSeekerLoggedIn && email !== 'guest') {
      if (isCurrentlySaved) {
        this.http.delete<any>(`http://localhost:3000/api/seeker/saved-jobs/${encodeURIComponent(email)}/${targetId}`).subscribe({
          error: (err) => console.warn('Background sync saved job delete error:', err)
        });
      } else {
        this.http.post<any>('http://localhost:3000/api/seeker/saved-jobs', { email, jobId: job.id || job._id }).subscribe({
          error: (err) => console.warn('Background sync saved job add error:', err)
        });
      }
    }
  }

  setSeekerFilterMode(mode: 'all' | 'saved') {
    this.seekerFilterMode = mode;
    this.applyFilters();
  }

  savedJobsCount(): number {
    return this.allJobs.filter(j => this.isJobSaved(j)).length;
  }

  viewSavedJobs() {
    this.seekerFilterMode = 'saved';
    this.applyFilters();
    this.scrollToJobs();
  }
  ...
  applyFilters() {
    ...
    if (this.isSeekerLoggedIn && this.seekerFilterMode === 'saved') {
      result = result.filter(job => this.isJobSaved(job));
    }
